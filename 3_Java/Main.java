...
public class Main {
    private static AnimalRegistry aManager;
    private static Scanner in;

    public static void main(String[] args) {
        aManager = new AnimalRegistry();
    ...
    protected static Animals getAnimal() {
        while (true) {
            try {
                System.out.println("Введите в формате: Type;Name;Birthday(YYYY-MM-DD);");
                return Animals.parse(in.nextLine());
            } catch (Exception e) {
                continue;
            }
        }
    ...
    protected static Animals getAnimalIndex() {
        while (true) {
            try {
    ...
    public static void printRegister() {
        ...
        System.out.println();
        for (Animals animal : aManager.getRegister()) {
            index += 1;
